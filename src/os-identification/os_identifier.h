...
struct mercury_record {
    ...
    void parse(struct datum &d) {
        ...
        if (d.skip_up_to_delim((const unsigned char *)"src_ip", sizeof("src_ip")-1) == false) return;
        if (d.accept('\"')) return;
        if (d.accept(':')) return;
        if (d.accept('\"')) return;
        ...
        if (d.skip_up_to_delim((const unsigned char *)"event_start", sizeof("event_start")-1) == false) return;
        if (d.accept('\"')) return;
        if (d.accept(':')) return;
