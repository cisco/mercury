...
namespace http_test {
    ...
    inline bool unit_test() {
        ...
        datum d1{(const uint8_t*)get_request, (const uint8_t*)get_request + strlen(get_request)};
        http_request req1{d1};
        if (!req1.is_not_empty()) return false;
        {
            buffer_stream buf{buffer, sizeof(buffer)};
            json_object json{&buf};
            req1.write_json(json, true);
            json.close();
            buf.write_char('\0');
            if (!strstr(buffer, "GET")) return false;
            if (!strstr(buffer, "example.com")) return false;
        }

        // Test HTTP POST request
        const char *post_request = "POST /api/data HTTP/1.1\r\nHost: api.test.com\r\nContent-Length: 0\r\n\r\n";
        datum d2{(const uint8_t*)post_request, (const uint8_t*)post_request + strlen(post_request)};
        http_request req2{d2};
        if (!req2.is_not_empty()) return false;

        // Test HTTP response
        const char *response = "HTTP/1.1 200 OK\r\nContent-Type: text/html\r\nContent-Length: 13\r\n\r\nHello, World!";
        datum d3{(const uint8_t*)response, (const uint8_t*)response + strlen(response)};
        http_response resp1{d3};
        if (!resp1.is_not_empty()) return false;
        {
            buffer_stream buf{buffer, sizeof(buffer)};
            json_object json{&buf};
            resp1.write_json(json, true);
            json.close();
            buf.write_char('\0');
            if (!strstr(buffer, "200")) return false;
        }

        // Test malformed request (too short)
        const char *bad_request = "GET";
        datum d4{(const uint8_t*)bad_request, (const uint8_t*)bad_request + strlen(bad_request)};
        http_request req3{d4};
        if (req3.is_not_empty()) return false;
