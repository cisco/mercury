...
public:
    ...
    traffic_selector(std::map<std::string, bool> protocols) : tcp{}, udp{}, select_tcp_syn{false}, select_dns{false}, select_nbns{false}, select_mdns{false} {
        ...
        if (protocols["ssdp"] || protocols["all"]) {
            udp.add_protocol(ssdp_notify::matcher_notify, udp_msg_type_ssdp);
            udp.add_protocol(ssdp_notify::matcher_search, udp_msg_type_ssdp);
            udp.add_protocol(ssdp_notify::matcher_response, udp_msg_type_ssdp); 
        }
