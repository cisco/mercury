...
class TestMercuryPython(unittest.TestCase):
    ...
    def test_perform_analysis_with_weights(self):
        ...
        cls_result2 = self.libmerc.perform_analysis_with_weights(str_repr, server_name, dst_ip, int(dst_port), '', 1, 0.1234, 0.00123, 0.456, 0.789, 0.5)
        cls_result3 = self.libmerc.perform_analysis_with_weights(str_repr, server_name, dst_ip, int(dst_port), '', 0.13924, 0.15590, 0.00528, 0.56735, 0.96941, 1.0)
        ...
        self.assertEqual(score1, score3,
                         f"Weight updates are not correct with perform_analysis_with_weights()")
        self.assertEqual(score1, 0.53339,
                         f"Feature weights from resource file is not read correctly, got {score1}")
